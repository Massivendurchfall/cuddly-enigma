...
local function captureFromContainer(container)
    local list={}
    ...
    for _,a in ipairs(container:GetChildren()) do
        if a:IsA("Animation") and a.AnimationId~="" then
            ...
            if num then table.insert(list,tonumber(num)) end
        end
    end
    return list
end
...
local function captureOriginalIds()
    ...
    local anim=getAnimateScript()
    if not anim then return end
    OriginalIds.idle=captureFromContainer(anim:FindFirstChild("idle"))
    OriginalIds.walk=captureFromContainer(anim:FindFirstChild("walk"))
    OriginalIds.run=captureFromContainer(anim:FindFirstChild("run"))
    OriginalIds.jump=captureFromContainer(anim:FindFirstChild("jump"))
    OriginalIds.fall=captureFromContainer(anim:FindFirstChild("fall"))
    OriginalIds.climb=captureFromContainer(anim:FindFirstChild("climb"))
    OriginalIds.swim=captureFromContainer(anim:FindFirstChild("swim"))
    OriginalIds.swimidle=captureFromContainer(anim:FindFirstChild("swimidle"))
    OriginalIdsCaptured=true
end

local function applyListToContainer(container,list)
    if not container or #list==0 then return end
    local anims={}
    for _,a in ipairs(container:GetChildren()) do
...
local function applyPack(packName)
    ...
    applyListToContainer(anim:FindFirstChild("idle"),pack.idle or {})
    applyListToContainer(anim:FindFirstChild("walk"),pack.walk or {})
    applyListToContainer(anim:FindFirstChild("run"),pack.run or {})
    applyListToContainer(anim:FindFirstChild("jump"),pack.jump or {})
    applyListToContainer(anim:FindFirstChild("fall"),pack.fall or {})
    applyListToContainer(anim:FindFirstChild("climb"),pack.climb or {})
    applyListToContainer(anim:FindFirstChild("swim"),pack.swim or {})
    applyListToContainer(anim:FindFirstChild("swimidle"),pack.swimidle or {})
    task.wait(0.05)
    refreshHumanoidState()
end

local function resetToOriginal()
    if not OriginalIdsCaptured then return end
    local char=LocalPlayer.Character
    if not char then return end
    local anim=char:FindFirstChild("Animate") or char:WaitForChild("Animate",10)
    if not anim then return end
    applyListToContainer(anim:FindFirstChild("idle"),OriginalIds.idle)
    applyListToContainer(anim:FindFirstChild("walk"),OriginalIds.walk)
    applyListToContainer(anim:FindFirstChild("run"),OriginalIds.run)
    applyListToContainer(anim:FindFirstChild("jump"),OriginalIds.jump)
    applyListToContainer(anim:FindFirstChild("fall"),OriginalIds.fall)
    applyListToContainer(anim:FindFirstChild("climb"),OriginalIds.climb)
    applyListToContainer(anim:FindFirstChild("swim"),OriginalIds.swim)
    applyListToContainer(anim:FindFirstChild("swimidle"),OriginalIds.swimidle)
    task.wait(0.05)
    refreshHumanoidState()
end

LocalPlayer.CharacterAdded:Connect(function(char)
    if ReapplyOnRespawn and CurrentPack and Packs[CurrentPack] then
        task.spawn(function()
...
Tabs.Anim:CreateButton({
    Name="Apply Pack",
    Callback=function()
        if CurrentPack and Packs[CurrentPack] then
            applyPack(CurrentPack)
            Rayfield:Notify({Title="Animations",Content="Pack applied.",Duration=5})
        else
            Rayfield:Notify({Title="Animations",Content="No valid pack selected.",Duration=5})
        end
    end
})

Tabs.Anim:CreateButton({
    Name="Reset default animations",
    Callback=function()
        resetToOriginal()
        Rayfield:Notify({Title="Animations",Content="Reset to default.",Duration=5})
    end
})

Tabs.Anim:CreateToggle({
    Name="Reapply on respawn",
    CurrentValue=true,
    Flag="ReapplyRespawn",
    Callback=function(v)
        ReapplyOnRespawn=v
...
local function playEmoteAsset(assetId,loop,speed)
    ...
    if not anim then
        Rayfield:Notify({Title="Emote",Content="Could not load asset.",Duration=5})
        return
    end
        ...
        task.spawn(function()
            while EmoteLoop and EmoteTrack do
                if not EmoteTrack.IsPlaying then
                    pcall(function()
                        EmoteTrack:Play(0.1,1,1)
                        EmoteTrack:AdjustSpeed(speed or 1)
                    end)
                end
                task.wait(0.25)
            end
