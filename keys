...
local function trackDoor(m)
    ...
    if pr then trackedDoorPrompts[pr]=true end
    tagDoor(m)
    ...
    if pr then
        pr.AncestryChanged:Connect(function(_,p) if not p then trackedDoorPrompts[pr]=nil end end)
        pr:GetPropertyChangedSignal("Enabled"):Connect(function() end)
    end
end
...
if maps then
    ...
    maps.DescendantRemoving:Connect(function(d)
        ...
        if trackedDoorPrompts[d] then trackedDoorPrompts[d]=nil end
    end)
end
...
local function availableKeys()
    ...
    for pr,_ in pairs(trackedKeyPrompts) do
        if alive(pr) and isValidKeyPrompt(pr) then
            ...
            if part and alive(part) then table.insert(res,{prompt=pr,part=part}) end
        end
    end
    return res
end
...
local function pressE(times,holdSeconds)
    ...
    for i=1,math.max(1,times) do
        ...
        if safetyEnabled and not safetyActive then
            local r=hrp(); if r and nearestMonsterDist(r.Position)<10 then return end
        end
    end
...
local function isJoinText(t)
    ...
    return t:find("join") or t:find("current match") or t:find("adventure is in progress") or t:find("beitreten") or t:find("spiel beitreten") or t:find("match beitreten")
end
local function visibleDeep(o)
    if not o or not o.Parent then return false end
    local cur=o
    while cur and cur~=o.Parent and cur~=game do
        if cur:IsA("GuiObject") and cur.Visible==false then return false end
        cur=cur.Parent
    end
    return true
end
local function centerPos(gui)
    local p=gui.AbsolutePosition
    local s=gui.AbsoluteSize
    return Vector2.new(p.X+s.X/2,p.Y+s.Y/2)
end
local function scoreButton(btn)
    if not btn or not btn.Parent then return -1 end
    if not visibleDeep(btn) then return -1 end
    local score=0
    if btn:IsA("TextButton") then
        score=score+2
        if isJoinText(btn.Text) then score=score+6 end
    elseif btn:IsA("ImageButton") then
        score=score+1
    end
    local lab=btn:FindFirstChildWhichIsA("TextLabel",true)
    if lab and isJoinText(lab.Text) then score=score+5 end
    if string.lower(btn.Name)=="joinbtn" then score=score+5 end
    return score
end
local function joinHeaderVisible()
    local pg=Players.LocalPlayer:FindFirstChild("PlayerGui"); if not pg then return false end
    if cachedJoinLabel and cachedJoinLabel.Parent and visibleDeep(cachedJoinLabel) then return true end
    local best=nil
    for _,d in ipairs(pg:GetDescendants()) do
        if d:IsA("TextLabel") and visibleDeep(d) and isJoinText(d.Text) then best=d break end
    end
    cachedJoinLabel=best
    return best~=nil
end
local function findJoinButtonSmart()
    local pg=Players.LocalPlayer:FindFirstChild("PlayerGui"); if not pg then return nil end
    local bestBtn=nil
    local bestScore=-1
    for _,d in ipairs(pg:GetDescendants()) do
        if (d:IsA("TextButton") or d:IsA("ImageButton")) and visibleDeep(d) then
            local sc=scoreButton(d)
            if sc>bestScore then bestScore=sc bestBtn=d end
        end
    end
    if not bestBtn then return nil end
    return bestBtn
end
local function fires(btn,ev)
    if not btn then return end
    if firesignal then pcall(function() firesignal(btn[ev],btn) end) end
end
local function robustClick(btn)
    if not btn or not btn.Parent then return end
    local pos=centerPos(btn)
    pcall(function() btn:Activate() end)
    fires(btn,"MouseButton1Down")
    fires(btn,"Activated")
    VirtualInputManager:SendMouseButtonEvent(pos.X,pos.Y,0,true,game,0)
    task.wait(0.03)
    fires(btn,"MouseButton1Up")
    fires(btn,"MouseButton1Click")
    VirtualInputManager:SendMouseButtonEvent(pos.X,pos.Y,0,false,game,0)
end
local function buttonIsTopMostAtCenter(btn)
    if not btn or not btn.Parent then return false end
    local pos=centerPos(btn)
    local arr=GuiService:GetGuiObjectsAtPosition(pos.X,pos.Y)
    if not arr or #arr==0 then return true end
    for _,o in ipairs(arr) do
        if o==btn or o:IsDescendantOf(btn) then return true end
        if o.Active and o.Visible then return o==btn end
    end
    return false
end
local function clickJoinButton()
    ...
    local t=time()
    if t<joinDebounceUntil then return end
    if t<nextScanAt then
        if cachedJoinBtn and cachedJoinBtn.Parent and visibleDeep(cachedJoinBtn) and joinHeaderVisible() then
            local sig=tostring(cachedJoinBtn:GetDebugId())..":"..tostring(cachedJoinBtn.AbsolutePosition)..":"..tostring(cachedJoinBtn.AbsoluteSize)
            if sig~=lastJoinSignature or not buttonIsTopMostAtCenter(cachedJoinBtn) then
                lastJoinSignature=sig
                robustClick(cachedJoinBtn)
                joinDebounceUntil=t+0.6
            end
        end
        return
    end
    ...
    cachedJoinBtn=findJoinButtonSmart()
    if cachedJoinBtn and visibleDeep(cachedJoinBtn) then
        local sig=tostring(cachedJoinBtn:GetDebugId())..":"..tostring(cachedJoinBtn.AbsolutePosition)..":"..tostring(cachedJoinBtn.AbsoluteSize)
        if sig~=lastJoinSignature or not buttonIsTopMostAtCenter(cachedJoinBtn) then
            lastJoinSignature=sig
            robustClick(cachedJoinBtn)
            joinDebounceUntil=time()+0.6
        end
    end
end
...
TabESP:CreateToggle({Name="Monster ESP",CurrentValue=false,Flag="ESP_Monster",Callback=function(s)
    monsterESPEnabled=s
    if s then
        for _,m in ipairs((monstersRoot() and monstersRoot():GetChildren()) or {}) do
            if isMonsterModel(m) then ensureHL(m,"KS_MonHL",Color3.fromRGB(255,60,60)) local head=m:FindFirstChild("Head") or m:FindFirstChild("HumanoidRootPart"); if head then ensureBillboard(head,"KS_MonBB",m.Name,Color3.fromRGB(255,60,60)) end end
        end
    else
        for _,m in ipairs((monstersRoot() and monstersRoot():GetChildren()) or {}) do
...
TabAuto:CreateToggle({Name="Auto-Join",CurrentValue=false,Flag="AutoJoin",Callback=function(s) autoJoin=s if not s then lastJoinSignature=nil cachedJoinBtn=nil cachedJoinLabel=nil nextScanAt=0 joinDebounceUntil=0 end end})
...
local PRE_DOOR_DELAY=1.2
local POST_DOOR_DELAY=0.6

local function spamNearbyPrompts(radius,pressTimes)
    radius=radius or 14
    pressTimes=pressTimes or 4
    local r=hrp(); if not r then return end
    local root=r.Position
    local near={}
    for pr,_ in pairs(trackedKeyPrompts) do
        local bp=nearestBasePartFrom(pr)
        if bp and (bp.Position-root).Magnitude<=radius then table.insert(near,pr) end
    end
    for i=1,pressTimes do
        for _,pp in ipairs(near) do pcall(function() firePrompt(pp,0.05) end) end
        pressE(1,0)
        task.wait(0.1)
    end
end
...
local function doAutoFarmStep()
    ...
    if key and key.part then
        ...
        task.wait(1.2)
        spamNearbyPrompts(14,5)
        pressE(6,0)
        task.wait(0.25)
    end
    local door=anyDoor()
    if door and door.part and door.prompt then
        holdDoorPrompt(door,1.2)
    end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if autoTicker>0.7 then
        autoTicker=0
        if autoFarm then doAutoFarmStep() end
    end

    if pumpkinTicker>0.9 then
        pumpkinTicker=0
        ...
        if playerESPEnabled then
            for pl,_ in pairs(trackedPlayers) do
                if pl~=LP and pl.Character then
                    if playerESPEnabled then ensureHL(pl.Character,"KS_PlayerHL",Color3.fromRGB(0,255,0)) end
                end
            end
        end
    end
                ...
                if alive(pr) and pr.Enabled and isDoorPromptInstance(pr) then
                    ...
                    if part then
                        ...
                        if d<=9 then
                            ...
                            if not instantOpenDebounce[key] or (time()-instantOpenDebounce[key])>1.0 then
                                if pr.HoldDuration and pr.HoldDuration>0 then
                                    firePrompt(pr,1.0)
                                else
                                    pressE(1,0.15)
                                end
                                instantOpenDebounce[key]=time()
                            end
                        end
                    end
                end
